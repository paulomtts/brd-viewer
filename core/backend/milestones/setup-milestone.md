...
This skill assumes a spec already exists — scope, the shape of the solution, the decisions already argued out. Do not invent scope here.

Breaking down is a *reading* exercise against that spec: every subtask must trace to something the spec asked for. If you find yourself deciding what the product should do while writing issue bodies, that is scope-setting leaking into breakdown — go back to the spec.
...
| Trap | Reality |
|---|---|
| Writing scope while breaking down | Breakdown reads a spec; it does not author one. |
