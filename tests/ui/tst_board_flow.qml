...
TestCase {
  ...
  function test_board_and_detail_flow() {
    ...
    p.activateCursor(); compare(p.app.board.selectedCardId, "m1")
    p.activateCursor(); compare(p.app.board.selectedCardId, "s1")
    compare(p.app.board.detailLinkList.map(function(l) { return l.section }).join(","), "parent,child,child")
    p.moveCursor(1); p.activateCursor(); compare(p.app.board.selectedCardId, "t1")
