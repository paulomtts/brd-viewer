...
TestCase {
  ...
  function test_memory_types_labels_colors() {
    compare(Logic.MEMORY_TYPES.map(function(t) { return t.id }).join(","), "user,feedback,project,reference,other")
    compare(Logic.memoryTypeLabel("feedback"), "Feedback")
    compare(Logic.memoryTypeLabel("nope"), "")
    compare(Logic.memoryTypeColor("user", "#111111"), "#d98cb3")
    compare(Logic.memoryTypeColor("other", "#111111"), "#111111")
  }

  function test_memory_type_counts_and_filters() {
    compare(Logic.memoryTypeCounts(memNotes).map(function(c) { return c.id + ":" + c.count }).join(","),
      "user:1,feedback:2,other:1")
    compare(Logic.filterMemoriesByType(memNotes, "feedback").length, 2)
    compare(Logic.filterMemoriesByType(memNotes, "").length, 4)
    compare(Logic.filterMemoriesByType(memNotes, undefined).length, 4)
    compare(Logic.filterMemoriesByType(memNotes, "project").length, 0)
  }

  function test_memory_search_matches_name_description_and_file() {
    compare(Logic.filterMemories(memNotes, "terse").length, 1)
    compare(Logic.filterMemories(memNotes, "DATABASE").length, 1)
    compare(Logic.filterMemories(memNotes, "user_role").length, 1)
    compare(Logic.filterMemories(memNotes, "  ").length, 4)
    compare(Logic.filterMemories(memNotes, "zzz").length, 0)
  }

  function test_parse_memories_result() {
    var ok = Logic.parseMemoriesResult('{"ok": true, "found": true, "memory_dir": "/m/memory", "notes": [{"file": "a.md", "name": "A", "description": "", "type": "user", "size": 1, "indexed": true}]}', 0)
    compare(ok.ok, true); compare(ok.found, true); compare(ok.memoryDir, "/m/memory"); compare(ok.notes.length, 1)
    var none = Logic.parseMemoriesResult('{"ok": true, "found": false, "memory_dir": "", "notes": []}', 0)
    compare(none.ok, true); compare(none.found, false)
    var bad = Logic.parseMemoriesResult('{"ok": false, "error": "nope"}', 1)
    compare(bad.ok, false); compare(bad.error, "nope"); compare(bad.notes.length, 0)
    compare(Logic.parseMemoriesResult("", 1).error, "Could not read this project's memories.")
    compare(Logic.parseMemoriesResult("garbage", 0).ok, false)
  }

  function test_parse_memory_op_result() {
    var ok = Logic.parseMemoryOpResult('{"ok": true, "backup": "/b"}', 0)
    compare(ok.ok, true); compare(ok.backup, "/b")
    var bad = Logic.parseMemoryOpResult('{"ok": false, "error": "exists"}', 1)
    compare(bad.ok, false); compare(bad.error, "exists")
    compare(Logic.parseMemoryOpResult("", 1).error, "Could not update the memory.")
    compare(Logic.parseMemoryOpResult('{"ok": true}', 1).ok, false)
  }

  function test_memory_absolute_path_and_new_file_names() {
    compare(Logic.memoryAbsolutePath("/m/memory", "a b.md"), "/m/memory/a b.md")
    compare(Logic.newMemoryFile("feedback", "Terse Replies!", []), "feedback_terse-replies.md")
    compare(Logic.newMemoryFile("feedback", "Terse Replies!", ["feedback_terse-replies.md"]), "feedback_terse-replies-2.md")
    compare(Logic.newMemoryFile("feedback", "Terse Replies!", ["feedback_terse-replies.md", "feedback_terse-replies-2.md"]), "feedback_terse-replies-3.md")
    compare(Logic.newMemoryFile("weird", "  ", []), "other_memory.md")
    compare(Logic.newMemoryFile("user", "../../etc/passwd", []), "user_etc-passwd.md")
    compare(Logic.newMemoryFile("project", "Ünïcode ✨ name", []), "project_n-code-name.md")
  }

  function test_compose_memory_builds_frontmatter() {
    compare(Logic.composeMemory("Terse", "no summaries", "feedback", "Body text\n"),
      "---\nname: Terse\ndescription: no summaries\nmetadata:\n  type: feedback\n---\n\nBody text\n")
    compare(Logic.composeMemory("A: b", 'say "hi" # x', "user", ""),
      '---\nname: "A: b"\ndescription: "say \\"hi\\" # x"\nmetadata:\n  type: user\n---\n\n')
    compare(Logic.composeMemory("Multi\nline", "d\r\ne", "project", "b"),
      "---\nname: Multi line\ndescription: d e\nmetadata:\n  type: project\n---\n\nb")
  }
}
