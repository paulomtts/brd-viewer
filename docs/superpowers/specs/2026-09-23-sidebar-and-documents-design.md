...
- Documents = the selected project's Markdown files: root `README.md` plus every
  `.md` under `docs/`.
...
`{"ok": false, "error": "..."}`. Each doc is `{"path", "title", "size"}`:

- `path` is relative to the project root with forward slashes (`README.md`,
  `docs/specs/x.md`). The panel builds the absolute path as
  `root_path + "/" + path`.
- Candidates: `README.md` at the root, and every `*.md` (case-insensitive
  extension) found recursively under `docs/`. Hidden directories are skipped.
...
- Order: `README.md` first, then remaining paths sorted case-insensitively.
